...
const Breadcrumbs: React.FC<BreadcrumbsProps & { router: any }> = ({ items, router }) => {
          ...
          <li key={index} className="flex items-center">
            ...
            <Link href={item.href}>
              <p
                ...
                style={{ fontSize: '18px', color: router && router.asPath && router.asPath.includes(item.href) ? '#FFC824' : '#0D0D0D' }}
              >
                {item.text}
              </p>
            </Link>
