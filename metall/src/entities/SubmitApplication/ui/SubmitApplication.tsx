...
const SubmitApplication: React.FC = () => {
    ...
    return (
        <>
            ...
            <Modal
                title={<div style={{ display: 'flex', justifyContent: 'center', fontSize: '32px', fontWeight: '800', lineHeight: '37.5px' }}>{isSubmitted ? 'Заявка принята' : 'Оставить заявку'}</div>}
                open={isModalOpen}
                onOk={handleOk}
            ...
            >
                {isSubmitted ? (
                    ...
                    <form onSubmit={handleSubmit} style={{ textAlign: 'center', maxWidth: '350px' }}>
                        <Input
                            addonBefore={
                                <Select
                                    value={countryCode}
